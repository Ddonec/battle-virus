...
function Table({ type, board, classNames }) {
   ...
   const classList = ["table-centered table-1"];
   ...
   for (let row = 0; row < 10; row++) {
      ...
      for (let cell = 0; cell < 10; cell++) {
         ...
         if (cell === 0)
            markers.push(
               <div key={0} className="marker marker-row no-events">
                  {row + 1}
               </div>
            );
         if (row === 0)
            markers.push(
               <div key={1} className="marker marker-col no-events">
                  {getChar(cell)}
               </div>
            );
         ...
         if (!type || winner === 0) {
            ...
            if ((ship && destroyedShip && ship != destroyedShip) || (ship && !destroyedShip)) {
               const elem = <Ship key={2} shipPosition={ship} classNames={type ? "no-events" : ""} />;
               markers.push(elem);
            }
         }

         if (destroyedShip) {
            const elem = <Ship key={2} shipPosition={destroyedShip} classNames={"ship-destroyed"} />;
            markers.push(elem);
         }

         const td = (
            <Cell key={cell} {...{ type, setCell, clickedCells }} x={cell} y={row}>
               {markers}
            </Cell>
         );
         cells.push(td);
